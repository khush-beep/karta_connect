...
import { adminDeleteStudent } from "@/lib/api/karta.functions";
...
import {
  Users, Search, UserCheck, ShieldAlert, ShieldCheck, Trash2, Loader2, Sparkles, Plus
} from "lucide-react";
...
function AdminStudentsPage() {
  ...
  async function handleRemoveRegistered(profileId: string, email: string) {
    ...
    try {
      const res = await deleteStudent({ data: { userId: profileId, email } });
      if (res.success) {
  ...
  async function handleWhitelistSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!email) return;
  ...
  async function toggleBlock(profileId: string, currentBlocked: boolean) {
    try {
  ...
  async function handleDeleteWhitelist(whitelistId: string) {
      ...
      <Card>
        <CardContent className="p-0">
          {filteredEntries.length === 0 ? (
            <div className="text-center py-12 text-sm text-muted-foreground">No students match the criteria.</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {filteredEntries.map((ent) => {
                  ...
                  return (
                    <TableRow key={ent.email} className="hover:bg-muted/30 transition-colors">
                      <TableCell className="font-semibold text-foreground">
                        {ent.registered ? (
                          <Link to="/admin/students/$id" params={{ id: ent.profileId }} className="hover:underline text-primary">
                            {ent.name}
                          </Link>
                        ) : (
                          <span>{ent.name}</span>
                      ...
                      <TableCell className="text-right space-x-2">
                        {ent.registered && ent.profileId && (
                          <>
                            <Button
                              onClick={() => toggleBlock(ent.profileId, ent.blocked)}
                              variant="outline"
                              size="sm"
                            ...
                            <Button
                              onClick={() => handleRemoveRegistered(ent.profileId, ent.email)}
                              variant="destructive"
                              size="sm"
                              className="h-8 flex items-center gap-1.5"
                            >
                        ...
                        {!ent.registered && ent.whitelistId && (
                          <Button
                            onClick={() => handleDeleteWhitelist(ent.whitelistId)}
                            variant="destructive"
                            size="sm"
                            className="h-8 flex items-center gap-1.5"
                          >
