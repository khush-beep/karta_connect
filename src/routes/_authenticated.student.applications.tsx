          ...
          {apps.map((app) => (
            <Card key={app.id} className="hover:shadow-sm transition-shadow">
              <CardContent className="p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                  <div className="flex gap-4">
                    <div className="h-12 w-12 border rounded bg-muted flex items-center justify-center overflow-hidden shrink-0 mt-1">
                      {app.job_post?.company?.logo_url ? (
                        <img src={app.job_post.company.logo_url} alt={app.job_post.company.name} className="h-full w-full object-cover" />
                      ) : (
                ...
                {app.cover_note && (
                  <div className="mt-4 border-t pt-3 flex gap-2 text-xs text-muted-foreground bg-muted/20 p-3 rounded-lg">
