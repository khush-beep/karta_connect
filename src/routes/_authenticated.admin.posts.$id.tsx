...
import {
  Loader2, ArrowLeft, Briefcase, MapPin, Calendar, Building2, User, FileText, ExternalLink, Trash2, Award
} from "lucide-react";
...
function AdminPostDetailsPage() {
  ...
  async function updateApplicantStatus(appId: string, newStatus: string) {
  ...
  function getResumeDownloadUrl(filePath: string) {
    if (!filePath) return "#";
            ...
            <Card>
              ...
              <CardContent className="flex flex-wrap gap-1.5">
                {post.required_skills.map((skill: string) => (
                  <span key={skill} className="bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-semibold capitalize">
                    {skill}
                  </span>
                ))}
              </CardContent>
                            ...
                            <TableCell>
                              {app.student ? (
                                <Link
                                  ...
                                  params={{ id: app.student.user_id }}
                                  className="font-bold text-primary hover:underline"
                                >
                                  {app.student.name}
                                </Link>
                              ) : (
                                <span className="font-semibold text-muted-foreground">Deleted Student</span>
                              )}
                            ...
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <select
                                  ...
                                  className="flex h-8 rounded-md border border-input bg-background px-2.5 py-0.5 text-xs shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring font-medium uppercase tracking-wider"
                                >
