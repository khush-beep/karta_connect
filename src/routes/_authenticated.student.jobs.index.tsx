...
import {
  Briefcase, Search, MapPin, Calendar, CheckCircle2, ChevronRight, Loader2, Sparkles, Filter
} from "lucide-react";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/student/jobs/")({
  validateSearch: (search: Record<string, unknown>): { type?: "job" | "internship" } => {
    return {
...
function StudentJobsPage() {
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    return (
      ...
      job.required_skills?.some((s: string) => s.toLowerCase().includes(query))
    );
  });
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredJobs.map((job) => (
            <Card key={job.id} className="flex flex-col hover:shadow-md transition-shadow cursor-pointer" onClick={() => setSelectedJob(job)}>
              ...
              <CardContent className="pb-3 flex-1">
                ...
                <div className="flex flex-wrap gap-1">
                  {job.required_skills?.slice(0, 3).map((skill: string) => (
                    <span key={skill} className="bg-muted px-2 py-0.5 rounded text-[10px] font-medium capitalize">
                      {skill}
                    </span>
                  ))}
              ...
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 border rounded bg-muted flex items-center justify-center overflow-hidden shrink-0">
                  {selectedJob.company?.logo_url ? (
                    <img src={selectedJob.company.logo_url} alt={selectedJob.company.name} className="h-full w-full object-cover" />
                  ) : (
