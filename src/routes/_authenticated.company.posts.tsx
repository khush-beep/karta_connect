...
import { Loader2, Plus, Calendar, MapPin, Briefcase, Trash2, Power, PowerOff, Sparkles, X } from "lucide-react";

export const Route = createFileRoute("/_authenticated/company/posts")({
  validateSearch: (search: Record<string, unknown>) => {
    return {
      type: (search.type as "job" | "internship") || undefined,
    };
  },
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <form onSubmit={handleCreatePost} className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {skills.map((skill) => (
                    <span key={skill} className="bg-primary/10 text-primary px-2.5 py-1 rounded-full text-xs font-semibold capitalize flex items-center gap-1">
                      {skill}
      ...
      {posts.length === 0 ? (
        ...
        <div className="space-y-4 max-w-4xl">
          {posts.map((post) => (
            <Card key={post.id} className={!post.active ? "opacity-60" : ""}>
              <CardContent className="p-6">
                ...
                {post.required_skills?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-4 pt-3 border-t">
                    {post.required_skills.map((skill: string) => (
                      <span key={skill} className="bg-muted px-2 py-0.5 rounded text-[10px] font-semibold capitalize text-muted-foreground">
                        {skill}
                      </span>
                    ))}
                  </div>
