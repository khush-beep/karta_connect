...
import {
  Building2, ArrowLeft, Globe, Mail, Phone, MapPin, Briefcase, Users, FileText, CheckCircle2, Loader2, Trash2
} from "lucide-react";
...
function AdminCompanyDetails() {
  ...
  async function toggleJobActive(jobId: string, currentActive: boolean) {
    try {
  ...
  async function changeAppStatus(appId: string, newStatus: string) {
    try {
      const { error } = await supabase
        .from("applications")
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <Link
          to="/admin/companies"
          className="inline-flex items-center gap-1 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors mb-4"
        >
      ...
      <Card className="shadow-sm">
        ...
        <CardContent className="p-0">
          {jobs.length === 0 ? (
            <div className="text-center py-8 text-sm text-muted-foreground">No listings have been posted by this company yet.</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableRow key={job.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell>
                      {job.active ? (
                        ...
                        <span className="text-xs border bg-muted text-muted-foreground px-2 py-0.5 rounded-full font-medium">
                          Inactive
                        </span>
                      )}
                    </TableCell>
                    <TableCell className="text-right space-x-2">
                      <Button
                        onClick={() => toggleJobActive(job.id, job.active)}
                        variant="outline"
                        size="sm"
                        className="h-8"
                      >
      ...
      <Card className="shadow-sm">
        ...
        <CardContent className="p-0">
          {applications.length === 0 ? (
            <div className="text-center py-8 text-sm text-muted-foreground">No candidate applications have been submitted yet.</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                    ...
                    <TableRow key={app.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <TableCell className="text-right space-x-2">
                        ...
                        <select
                          ...
                          className="h-8 rounded-md border border-input bg-background px-2 py-0.5 text-xs shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring font-medium"
                        >
