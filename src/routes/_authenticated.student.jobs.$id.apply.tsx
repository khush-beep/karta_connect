      ...
      <Card>
        ...
        <CardContent className="pt-6 space-y-6">
          ...
          {(isProfileIncomplete || isResumeMissing) && (
            <div className="flex items-start gap-3 bg-destructive/10 text-destructive border border-destructive/20 p-4 rounded-xl text-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid gap-2">
              ...
              <textarea
                id="cover-note"
                placeholder="Describe why you are interested in this role and why you are a good fit..."
                value={coverNote}
                onChange={(e) => setCoverNote(e.target.value)}
                className="flex min-h-[140px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                disabled={isProfileIncomplete || isResumeMissing}
              />
            </div>
