...
import {
  Users, Building2, Briefcase, GraduationCap, FileText, Plus, Search,
  Calendar, CheckCircle2, Loader2, Sparkles, TrendingUp, UserCheck,
  Globe
} from "lucide-react";
import {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <Link to="/company/posts" search={{ type: "job" } as any}>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="h-full shadow-sm">
            ...
            <CardContent className="space-y-4 pt-4">
              {jobs.length === 0 ? (
                <div className="text-center py-8 text-sm text-muted-foreground">No active postings available right now.</div>
              ) : (
                jobs.map((job) => (
                  <div key={job.id} className="flex flex-col sm:flex-row justify-between border-b pb-4 last:border-0 last:pb-0 gap-3">
                    <div>
        ...
        <div className="lg:col-span-1">
          <Card className="h-full shadow-sm border border-border/80">
            ...
            <CardContent className="space-y-4 pt-4">
              {companies.length === 0 ? (
                <div className="text-center py-8 text-sm text-muted-foreground">No partner companies active at this time.</div>
              ) : (
                companies.map((c) => {
                  ...
                  return (
                    <div key={c.id} className="flex items-center justify-between border-b pb-3 last:border-0 last:pb-0 gap-3">
