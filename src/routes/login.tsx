...
import { resolveLogin } from "@/lib/api/karta.functions";
...
function LoginPage() {
  ...
  const [emailStatus, setEmailStatus] = useState<
    { state: "idle" }
    | { state: "invalid" }
    | { state: "valid"; hasPassword: boolean }
  >({ state: "idle" });
  ...
  async function checkEmail(value?: string) {
    ...
    setChecking(true);
    try {
      const info = await resolve({ data: { email: v } });
      if (info.kind === null) {
        setEmailStatus({ state: "invalid" });
      } else {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!emailStatus.hasPassword) {
      navigate({ to: "/create-password", search: { email } as any });
      return;
    }
    setLoading(true);
    try {
  ...
  async function goCreatePassword() {
    ...
    const info = await resolve({ data: { email } });
    if (info.kind === null) return toast.error("Invalid Email");
    if (info.hasPassword) return toast.error("Password already exists. Use Forgot Password to reset.");
    navigate({ to: "/create-password", search: { email } as any });
  }
        ...
        <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-2xl animate-in fade-in slide-in-from-right duration-700">
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (emailStatus.state !== "idle") setEmailStatus({ state: "idle" });
                }}
                onBlur={() => { if (email) checkEmail(); }}
                required
                aria-invalid={emailInvalid}
                className="focus-visible:ring-primary focus-visible:border-primary"
              />
              {emailInvalid && (
                <p className="mt-1 text-xs text-destructive font-medium">Invalid Email — not registered with Karta.</p>
              )}
              {emailValid && !emailStatus.hasPassword && (
            ...
            {!emailValid ? (
              <Button 
                type="button" 
                className="w-full font-bold shadow-md bg-zinc-950 text-zinc-50 hover:bg-zinc-900 focus-visible:ring-zinc-950 dark:bg-zinc-50 dark:text-zinc-950 dark:hover:bg-zinc-200 transition-colors" 
