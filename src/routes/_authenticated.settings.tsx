...
import { deleteOwnAccount } from "@/lib/api/karta.functions";
...
import {
  User, Lock, Sun, Moon, Trash2, Loader2, RefreshCw
} from "lucide-react";
...
function SettingsPage() {
  ...
  const [passwordMode, setPasswordMode] = useState<"normal" | "recovery-input" | "recovery-verify" | "recovery-reset">("normal");
  ...
  async function handleDeleteAccount() {
    ...
    try {
      const res = await deleteAccount({ data: { userId: user.id } });
      if (res.success) {
            ...
            {passwordMode === "normal" && (
              <form onSubmit={handlePasswordUpdate} className="space-y-4">
                <div className="grid gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setPasswordMode("recovery-input")}
                    className="text-left text-xs font-semibold text-primary hover:underline w-fit mt-1"
                  >
                    Forgot Password?
                  </button>
                </div>
