...
import { Briefcase, GraduationCap, Search, Trash2, Loader2, MapPin, Calendar, Power, PowerOff, FileText, ChevronRight } from "lucide-react";
...
function AdminPostsPage() {
    ...
    async function handleDelete(postId) {
        const confirmDelete = window.confirm("Are you sure you want to delete this listing? This will also remove all candidate applications associated with it.");
        if (!confirmDelete)
            return;
        try {
    ...
    return (<div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search postings by role, company, skills, location..." className="pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <label htmlFor="type-filter" className="text-xs text-muted-foreground font-medium">Type:</label>
            <select id="type-filter" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="flex h-9 rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring font-medium">
      ...
      <Card>
        <CardContent className="p-0">
          {filteredPosts.length === 0 ? (<div className="text-center py-12 text-sm text-muted-foreground font-medium">
              ...
              <TableBody>
                ...
                return (<TableRow key={post.id} className="hover:bg-muted/30 transition-colors">
                      <TableCell>
                        <div>
                          <Link to="/admin/posts/$id" params={{ id: post.id }} className="font-bold text-foreground hover:underline text-primary flex items-center gap-1">
                      ...
                      <TableCell className="text-sm font-medium">
                        <Link to="/admin/posts/$id" params={{ id: post.id }} className="hover:underline">
                      ...
                      <TableCell className="text-right space-x-2">
                        <Button onClick={() => toggleActive(post.id, post.active)} variant="outline" size="sm" className="h-8" title={post.active ? "Deactivate listing" : "Activate listing"}>
                          {post.active ? (<PowerOff className="h-3.5 w-3.5 text-amber-500"/>) : (<Power className="h-3.5 w-3.5 text-emerald-500"/>)}
                        </Button>
                        <Button onClick={() => handleDelete(post.id)} variant="destructive" size="sm" className="h-8" title="Delete listing">
