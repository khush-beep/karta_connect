...
function QuoteRotator() {
    ...
    return (<div className="flex flex-col justify-center h-full p-8 text-white space-y-6">
...
function Landing() {
    return (<div className="relative min-h-screen flex flex-col overflow-hidden bg-slate-950 font-sans">
      <BackgroundSlideshow />
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="relative w-full overflow-hidden">
            ...
            <div className="animate-marquee flex gap-8 items-center py-1.5">
              {partners.map((partner, i) => (<div key={i} className="flex items-center justify-center bg-white/5 px-5 py-2.5 rounded-xl border border-white/10 shadow-sm min-w-[150px] h-[48px] shrink-0">
                  {partner.isImage ? (<img src={partner.logo} alt={partner.name} className="max-h-[26px] w-auto object-contain brightness-0 invert opacity-75"/>) : (<span className="font-bold text-xs tracking-tight text-white/70 uppercase">{partner.name}</span>)}
                </div>))}
              {/* Duplicate set for seamless looping */}
              {partners.map((partner, i) => (<div key={`dup-${i}`} className="flex items-center justify-center bg-white/5 px-5 py-2.5 rounded-xl border border-white/10 shadow-sm min-w-[150px] h-[48px] shrink-0">
                  {partner.isImage ? (<img src={partner.logo} alt={partner.name} className="max-h-[26px] w-auto object-contain brightness-0 invert opacity-75"/>) : (<span className="font-bold text-xs tracking-tight text-white/70 uppercase">{partner.name}</span>)}
                </div>))}
            </div>
          </div>
